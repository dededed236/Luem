...
#include "ns3/drop-tail-queue.h"  // เพิ่มการนำเข้า DropTailQueue
...
int main(int argc, char *argv[])
{
    ...
    d02 = pp2.Install(nodes.Get(0), nodes.Get(1));
    d12 = pp2.Install(nodes.Get(1), nodes.Get(2));
    ...
    // Add DropTailQueue to PointToPoint devices
    for (int i = 0; i < NUMBER_OF_BOTS; ++i)
    {
        Ptr<PointToPointNetDevice> dev = DynamicCast<PointToPointNetDevice>(botDeviceContainer[i].Get(0));
        Ptr<DropTailQueue<Packet>> dropQueue = CreateObject<DropTailQueue<Packet>>();
        dropQueue->SetMaxPackets(5);  // ปรับขนาดคิวตามที่ต้องการ
        dev->SetQueue(dropQueue);
    }
