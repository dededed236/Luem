...
int main(int argc, char *argv[])
{
    ...
    // Legitimate connection nodes (2 nodes)
    NodeContainer nodes;
    nodes.Create(2);  // ลดโหนดปกติให้เหลือ 2 โหนด
    ...
    NetDeviceContainer d01, d12, botDeviceContainer[NUMBER_OF_BOTS], extraDeviceContainer[NUMBER_OF_EXTRA_NODES];
    d01 = pp2.Install(nodes.Get(0), nodes.Get(1));  // โหนดที่ 0 ต่อกับโหนดที่ 1
    ...
    {
        // Extra nodes connected to Node 0 and Node 1
        if (i < 3)
        {
    ...
    OnOffHelper onoff("ns3::UdpSocketFactory", Address(InetSocketAddress(i01.GetAddress(1), UDP_SINK_PORT)));
    ...
    // BulkSend on extra nodes to send TCP data to node 1
    OnOffHelper onoffTcp("ns3::TcpSocketFactory", Address(InetSocketAddress(i01.GetAddress(1), TCP_SINK_PORT)));
    ...
    ApplicationContainer UDPSinkApp = UDPsink.Install(nodes.Get(1));  // ติดตั้งบนโหนดที่ 1
    ...
    ApplicationContainer TCPSinkApp = TCPsink.Install(nodes.Get(1));  // ติดตั้งบนโหนดที่ 1
    ...
    ns3::AnimationInterface::SetConstantPosition(nodes.Get(1), 160, 45);  // วางตำแหน่งของโหนดที่ 1
